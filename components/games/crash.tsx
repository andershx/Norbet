...
export function Crash() {
  ...
  const [mult, setMult] = useState<number>(2.0);
  ...
  async function cashout() {
    ...
    const r = await fetch('/api/games/crash/cashout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ at: progress, wager }),
    });
    const data = await r.json();
    if (data?.payout > 0) celebrate(data.payout);
    setState('idle');
  }
  ...
  return (
    <div className="card space-y-3">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <label className="space-y-1">
          <span className="text-xs text-white/60">Auto Cashout</span>
          <input
            className="input w-full"
            type="number"
            step={0.1}
            value={Number.isFinite(mult) ? mult : 0}
            onChange={(e) => setMult(parseFloat(e.target.value || '0') || 0)}
          />
        </label>
        <div className="flex gap-2">
          <button onClick={start} className="btn">Start</button>
          <button onClick={cashout} className="btn btn-outline">Cash Out</button>
        </div>
      </div>
